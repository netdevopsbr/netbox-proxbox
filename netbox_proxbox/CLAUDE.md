...
- [`anonymize.py`](./anonymize.py): pure `re` + stdlib scrubber for that payload. Replaces credentials, URLs, e-mails, realm principals, MACs, IPv4/IPv6, and allowlisted-suffix FQDNs with **stable** per-instance placeholders (`<host-1>`, `<ip-2>`) so a report stays correlatable without naming the estate. **Must never import Django** — `tests/test_bug_report.py` exec-loads its consumer without one. Takes its credential vocabulary from `redaction.py`.
